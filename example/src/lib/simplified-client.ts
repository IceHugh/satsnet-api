// 与原始 satsnet-client.ts 一致的简化实现
// 参考 @src/utils/advanced-http.ts 的 undici 使用模式
// 专门测试 getNameInfo 接口在 Next.js 中的兼容性
...
// 简化的 HTTP 客户端 - 参考 advanced-http.ts 的核心实现
class NextJSHttpClient {
  ...
  /**
   * 构建 URL - 参考 advanced-http.ts 的 buildUrlWithCacheKey 方法
   */
  ...
  /**
   * GET 请求 - 参考 advanced-http.ts 的 get 方法
   */
  async get<T = any>(path: string): Promise<T> {
    ...
    try {
      ...
      // 安全解析 JSON - 参考 advanced-http.ts 的 safeParseJson 方法
      let data: any;
      try {
...
export class SatsNetClient {
  ...
  /**
   * Get name info by name - 与原始代码保持一致的实现
